...
if( $_GET['action'] == 'DisplayAllStatus' ){
					...
					<div class="widget-summary">
						<div class="widget-summary-col">
							...
							<div class="summary-footer">
								<a href="master_status_detail.html?id='.$row['id'].'" class="text-muted text-uppercase">( Lihat Detail )</a>
							</div>
						</div>
					</div>
				</div>
...
else if( $_GET['action'] == 'DisplayAllStatusDetail' ){
	...
	$query = "SELECT * FROM public.tab_status where id = '".$id."' ";
	$result = pg_query($conn, $query);
	while($row = pg_fetch_assoc($result)){
		...
		<div class="form-group row">
			...
			<div class="col-lg-6">
				<input type="text" class="form-control" id="input_namastatus" value="'.rtrim($row['nama']).'">
			</div>
		</div>
...
else if( $_GET['action'] == 'DisplayStatusFormAdd' ){
	...
	$query_check = "select * from public.tab_status where nama = '".$nama."' and id_rs = '".$idrs."'";
