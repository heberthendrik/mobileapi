...
if( $_GET['action'] == 'DisplayAllDokter' ){
				...
				<div class="card-body">
					<div class="widget-summary">
						<div class="widget-summary-col">
							<div class="summary">
								<h4 class="title">'.$row['nama'].' - <span style="display:block;" class="text-primary">('.$row['kode'].')</span></h4>
								<div class="info">
									<strong class="amount">'.$row['nama'].'</strong>
								</div>
							</div>
							<div class="summary-footer">
								<a href="master_dokter_detail.html?id='.$row['id'].'" class="text-muted text-uppercase">( Lihat Detail )</a>
							</div>
						</div>
					</div>
				</div>
...
else if( $_GET['action'] == 'DisplayAllDokterDetail' ){
	...
	$query = "SELECT * FROM public.tab_dokter where id = '".$id."' ";
	$result = pg_query($conn, $query);
	while($row = pg_fetch_assoc($result)){
		...
		<div class="form-group row">
			...
			<div class="col-lg-6">
				<input type="text" class="form-control" id="input_namadokter" value="'.rtrim($row['nama']).'">
			</div>
		</div>
...
else if( $_GET['action'] == 'DisplayDokterFormAdd' ){
	...
	<div class="form-group row">
		<label class="col-lg-3 control-label text-lg-right pt-2" for="inputDefault">Nama Dokter *</label>
		<div class="col-lg-6">
			<input type="text" class="form-control" id="input_namadokter" value="'.$row['nama'].'">
		</div>
	</div>
...
else if( $_GET['action'] == 'UpdateDokter' ){
	...
	$query_check = "select * from public.tab_dokter where nama = '".$nama."' and id_rs = '".$idrs."'";
