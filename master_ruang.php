...
if( $_GET['action'] == 'DisplayAllRuang' ){
...
else if( $_GET['action'] == 'DisplayAllRuangDetail' ){
	...
	$query = "SELECT * FROM public.tab_ruang where id = '".$id."' ";
	$result = pg_query($conn, $query);
	while($row = pg_fetch_assoc($result)){
		...
		<div class="form-group row">
			...
			<div class="col-lg-6">
				<input type="text" class="form-control" id="input_namaruang" value="'.rtrim($row['nama']).'">
			</div>
		</div>
...
else if( $_GET['action'] == 'DisplayRuangFormAdd' ){
	...
	<div class="form-group row">
		<label class="col-lg-3 control-label text-lg-right pt-2" for="inputDefault">Nama Ruang *</label>
		<div class="col-lg-6">
			<input type="text" class="form-control" id="input_namaruang" value="'.$row['nama'].'">
		</div>
	</div>
...
else if( $_GET['action'] == 'UpdateRuang' ){
	...
	$query_check = "select * from public.tab_ruang where nama = '".$nama."' and id_rs = '".$idrs."'";
