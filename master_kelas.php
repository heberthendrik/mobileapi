...
if( $_GET['action'] == 'DisplayAllKelas' ){
				...
				<div class="card-body">
					<div class="widget-summary">
						<div class="widget-summary-col">
							<div class="summary">
								<h4 class="title">'.$row['namars'].'</h4>
								<div class="info">
									<strong class="amount">'.$row['nama'].'</strong>
								</div>
							</div>
							<div class="summary-footer">
								<a href="master_kelas_detail.html?id='.$row['id'].'" class="text-muted text-uppercase">( Lihat Detail )</a>
							</div>
						</div>
					</div>
				</div>
...
else if( $_GET['action'] == 'DisplayAllKelasDetail' ){
	...
	$query = "SELECT * FROM public.tab_kelas where id = '".$id."' ";
	$result = pg_query($conn, $query);
	while($row = pg_fetch_assoc($result)){
		...
		<div class="form-group row">
			...
			<div class="col-lg-6">
				<input type="text" class="form-control" id="input_namakelas" value="'.rtrim($row['nama']).'">
			</div>
		</div>
...
else if( $_GET['action'] == 'DisplayKelasFormAdd' ){
	...
	<div class="form-group row">
		<label class="col-lg-3 control-label text-lg-right pt-2" for="inputDefault">Nama Kelas *</label>
		<div class="col-lg-6">
			<input type="text" class="form-control" id="input_namakelas" value="'.$row['nama'].'">
		</div>
	</div>
...
else if( $_GET['action'] == 'UpdateKelas' ){
	...
	$query_check = "select * from public.tab_kelas where nama = '".$nama."' and id_rs = '".$idrs."'";
